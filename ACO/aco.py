    ...
    def get_probabilities(self, start_node_index, unvisited_nodes):
        ...
        for node in range(len(unvisited_nodes)):
            ...
            probability = ((self.pheromone[start_node_index][next_node_index]**self.alpha
                            *self.visibility[start_node_index][next_node_index]**self.beta)*self.connection[start_node_index][next_node_index]/denominator)
        ...
        for i in range(iterations):
            ...
            for b in range(n_ants):
                ...
                while len(self.unvisited_nodes) != 0:
                    self.get_probabilities(current_node_index,self.unvisited_nodes)
                    max_prob = max(self.probabilities)
                    max_prob_index = self.probabilities.index(max_prob)
                    next_node = self.unvisited_nodes[max_prob_index]
